...
describe('reset password actions', () => {
    ...
    it('should display modal and reset password', () => {
        ...
        cy.get('#blui-password .blui-password').invoke('prop', 'type').should('contain', 'password');
        cy.get('#blui-password .blui-password').click().type('Test321!');
        cy.get('#blui-password .blui-visibility-icon').first().click();
        cy.get('#blui-password .blui-password').invoke('prop', 'type').should('contain', 'text');
    ...
    it('should clear forgot password modal on navigation', () => {
        ...
        cy.visit('http://localhost:4200/auth/reset-password?code=DEADBEEF&email=resetPassword@email.com');
        cy.get('#blui-password .blui-password').click().should('not.contain', 'Test321!');
    });
});
